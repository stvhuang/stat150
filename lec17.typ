...
*Eg:* Gambler's Ruin, $P_x("Jackpot")$ & $P_x("Ruin")$ is the exit distribution from $ {1, 2, ..., N-1} $ started at $x$.
...
Diagram of states $0, ..., i, ..., N$. From state $i$, transition to $i-1$ with probability $q$ and to $i+1$ with probability $p$.
...
For $i in {1, 2, ..., N-1}$,
$
  h(i) = sum_j p_(i j) h(j) = q h(i-1) + p h(i+1)
$
is what we got by FSA on HW #1.

#pagebreak()

Theorem

Let $(X_n)$ be a MC on $S$. Suppose $A, B subset S$ such that $C = S backslash (A union B)$ is finite.
...
Then,
$ h(i) = P_i(V_A < V_B) $ if $P_i(min{V_A, V_B} < oo) > 0$
for all $i in C$.

*Proof:* Essentially FSA, but more technical, see [p]. $square$
...
Look at Ex 1.42 [p] "Matching Pennies" on your own.
...
See ex 1.46 [p] for a nice example.
...
2. SD $pi$ for $P$ restricted to $C$.

Then $lim_(n->oo) p_(i j)^n = pi_j P("hit C 1st")$

#pagebreak()

// State diagram with nodes {1, ..., 7}.
// The closed communicating classes are A = {1, 5} and B = {4, 6, 7}.
// The class {2, 3} is transient.
// Transitions exist from the transient class to the closed classes.
...
// State diagram with transient states {2, 3} and absorbing states {A, B}.
// Transitions from 2: to A (0.1), to 2 (0.2), to 3 (0.3), to B (0.4)
// Transitions from 3: to A (0.2), to 3 (0.5), to B (0.3)
