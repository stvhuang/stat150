...
1. $N_0 = 0$
2. Poisson increments: \ $(N_(t+s) - N_s)#footnote[\# pts in $(t, t+s]$] ~ "Poisson"(lambda t#footnote[length of $= (s, t+s]$])$, for any $s, t$
3. Indep. increments: \ $N_(t_1) - N_(t_0), dots, N_(t_n) - N_(t_(n-1))$ indep for any $t_0 < t_1 < dots < t_(n-1) < t_n$

#pagebreak()

Construction: \
"Point process" on $[0, oo)$ \
with IID $"Exp"(lambda)$ inter-arrival times between points.
...
(§2.4 in [D])
Thinning of Poisson Processes#footnote[taking a PP, and spliting it up into indep. sub. PPs]
...
#text(size: 10pt)[
  $
    ...
    & = product_(j=1)^m e^(-lambda p_j t) ((lambda p_j t)^(k_j)) / (k_j !) = underbrace(product_(j=1)^m, #text[indep.]) P("Poisson"(lambda p_j) = k_j)
  $
]
...
3. Indep. increments#footnote[condition (1.) and (3.) are the same as for regular (hom.) PP]
